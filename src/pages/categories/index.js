...
import { destroy, updateStatus } from '../../api/categories';
...
function Index(props) {
    ...
    const urls = {
        allData: 'categories/all-categories',
    ...
    const action = item => {
        return (
            <Dropdown.Menu as={CustomMenu}>
                <Dropdown.Item
                    onClick={navigateTo.bind(this, `/categories/edit/${item.id}`)}
                    eventKey="1">
                    <span className="mr-3">
    ...
    const row = item => {
        return (
            <tr key={item.id}>
                ...
                <td>
                    <NavLink to={`/categories/edit/${item.id}`}>
                        <b>{item.title}</b>
                    </NavLink>
                </td>
                <td
                ...
                <td>{item.description ? item.note : 'N/A'}</td>
    ...
    const handleChangeStatus = item => {
        ...
        const swalSetting = {
            ...
            text: `Bạn có muốn <b>${text1}</b>  thể loại này ?<br>${item.status === 'show' ? 'Các mục con trong thể loại này sẽ không được hiển thị' : ''}`,
        ...
        <Auxiliary>
            ...
            <div className="page-content fade-in-up">
                <div className="ibox">
                    <BoxHead>
                        <div
                            onClick={navigateTo.bind(this, '/categories/sort-categories')}
                            className="btn btn-gradient-aqua btn-rounded btn-fix mr-1">
                            <i className={'fa fa-sort mr-1'} /> Sắp xếp
                        </div>
                        <div
                            onClick={navigateTo.bind(this, '/categories/create')}
